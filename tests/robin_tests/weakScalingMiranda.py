### USAGE
### weakScalingMiranda.py <mode> <maxentend> <#threads> <#max_threads>
...
zextend = int(int(extent) * (int(numthreads)/int(maxNumthreads)))

# create a new 'Extract Subset'
data = ExtractSubset(registrationName='subdata', Input=data)
data.VOI = [0, extent-1, 0, extent-1, 0, zextend-1]
...
#     SaveScreenshot('/data/img/' + datasetname + '_' + sys.argv[1] + '_' + numthreads + '_' + sys.argv[2] + '.png', layout, SaveAllViews=1, ImageResolution=[1920,1080])
