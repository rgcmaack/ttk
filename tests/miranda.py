...
print( f"miranda benchmark {algorithm}, size {extent}, {nthreads} threads" )
...
# data = ImageReader(registrationName='miranda_1024x1024x1024_float32.raw', FileNames=['/data/miranda_1024x1024x1024_float32.raw'])
# data.DataScalarType = 'float'
# data.DataByteOrder = 'LittleEndian'
# data.DataExtent = [0, 1023, 0, 1023, 0, 1023]

data = XMLImageDataReader(registrationName='miranda_ps_0.01.vti', FileName=['/data/miranda_ps_0.01.vti'])
data.PointArrayStatus = ['ImageFile']

# create a new 'Extract Subset'
data = ExtractSubset(registrationName='subdata', Input=data)
data.VOI = [0, extent-1, 0, extent-1, 0, extent-1]
...
if sys.argv[1] == 'msc':
    ...
    msspl.UseAllCores = 0
    msspl.ThreadNumber = nthreads
    msspl.a2SeparaticiesMode = "Separate Basins fast"
...
#     SaveScreenshot( f'miranda-{algorithm}-{extent}-{nthreads}T.png', layout, SaveAllViews=1, ImageResolution=[1920,1080])
