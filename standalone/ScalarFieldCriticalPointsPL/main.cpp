...
#include <ttkScalarFieldCriticalPointsPL.h>
...
int main(int argc, char **argv) {
  ...
  // ---------------------------------------------------------------------------
  ttk::Debug msg;
  msg.setDebugMsgPrefix("ScalarFieldCriticalPointsPL");

  // ---------------------------------------------------------------------------
  // Initialize ttkScalarFieldCriticalPointsPL module (adjust parameters)
  // ---------------------------------------------------------------------------
  auto scalarFieldCriticalPointsPL
    = vtkSmartPointer<ttkScalarFieldCriticalPointsPL>::New();
  ...
  for(size_t i = 0; i < inputFilePaths.size(); i++) {
    ...
    if(listArrays) {
      ...
      // feed input object to ttkScalarFieldCriticalPointsPL filter
      scalarFieldCriticalPointsPL->SetInputDataObject(i, reader->GetOutput());
  ...
  for(size_t i = 0; i < inputArrayNames.size(); i++)
    scalarFieldCriticalPointsPL->SetInputArrayToProcess(
      i, 0, 0, 0, inputArrayNames[i].data());

  // ---------------------------------------------------------------------------
  // Execute ttkScalarFieldCriticalPointsPL filter
  // ---------------------------------------------------------------------------
  scalarFieldCriticalPointsPL->Update();

  // ---------------------------------------------------------------------------
  // If output prefix is specified then write all output objects to disk
  // ---------------------------------------------------------------------------
  if(!outputPathPrefix.empty()) {
    for(int i = 0; i < scalarFieldCriticalPointsPL->GetNumberOfOutputPorts();
        i++) {
      auto output = scalarFieldCriticalPointsPL->GetOutputDataObject(i);
      auto writer
