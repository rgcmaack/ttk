...
int main(int argc, char **argv) {
  ...
  // ---------------------------------------------------------------------------
  ttk::Debug msg;
  msg.setDebugMsgPrefix("morseSmaleComplexOrder");

  // ---------------------------------------------------------------------------
  // Initialize ttkmorseSmaleComplexOrder module (adjust parameters)
  // ---------------------------------------------------------------------------
  auto morseSmaleComplexOrder = vtkSmartPointer<ttkmorseSmaleComplexOrder>::New();

  // ---------------------------------------------------------------------------
  // TODO 14: Pass custom arguments and options to the module
  // ---------------------------------------------------------------------------
  // morseSmaleComplexOrder->SetOutputArrayName(outputArrayName);
  ...
  for(size_t i = 0; i < inputFilePaths.size(); i++) {
    ...
    if(listArrays) {
      ...
      // feed input object to ttkmorseSmaleComplexOrder filter
      morseSmaleComplexOrder->SetInputDataObject(i, reader->GetOutput());
  ...
  for(size_t i = 0; i < inputArrayNames.size(); i++)
    morseSmaleComplexOrder->SetInputArrayToProcess(i, 0, 0, 0, inputArrayNames[i].data());

  // ---------------------------------------------------------------------------
  // Execute ttkmorseSmaleComplexOrder filter
  // ---------------------------------------------------------------------------
  morseSmaleComplexOrder->Update();

  // ---------------------------------------------------------------------------
  // If output prefix is specified then write all output objects to disk
  // ---------------------------------------------------------------------------
  if(!outputPathPrefix.empty()) {
    for(int i = 0; i < morseSmaleComplexOrder->GetNumberOfOutputPorts(); i++) {
      auto output = morseSmaleComplexOrder->GetOutputDataObject(i);
      auto writer
