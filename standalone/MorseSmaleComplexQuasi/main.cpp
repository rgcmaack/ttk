...
/// \author Robin G. C. Maack
/// \date June 2021
///
/// \brief Command line program for order Morse-Smale complex computation.
...
int main(int argc, char **argv) {
  ...
  // ---------------------------------------------------------------------------
  ttk::Debug msg;
  msg.setDebugMsgPrefix("morseSmaleComplexQuasi");

  // ---------------------------------------------------------------------------
  // Initialize ttkmorseSmaleComplexQuasi module (adjust parameters)
  // ---------------------------------------------------------------------------
  auto morseSmaleComplexQuasi = vtkSmartPointer<ttkmorseSmaleComplexQuasi>::New();

  // ---------------------------------------------------------------------------
  // TODO 14: Pass custom arguments and options to the module
  // ---------------------------------------------------------------------------
  // morseSmaleComplexQuasi->SetOutputArrayName(outputArrayName);
  ...
  for(size_t i = 0; i < inputFilePaths.size(); i++) {
    ...
    if(listArrays) {
      ...
      // feed input object to ttkmorseSmaleComplexQuasi filter
      morseSmaleComplexQuasi->SetInputDataObject(i, reader->GetOutput());
  ...
  for(size_t i = 0; i < inputArrayNames.size(); i++)
    morseSmaleComplexQuasi->SetInputArrayToProcess(i, 0, 0, 0, inputArrayNames[i].data());

  // ---------------------------------------------------------------------------
  // Execute ttkmorseSmaleComplexQuasi filter
  // ---------------------------------------------------------------------------
  morseSmaleComplexQuasi->Update();

  // ---------------------------------------------------------------------------
  // If output prefix is specified then write all output objects to disk
  // ---------------------------------------------------------------------------
  if(!outputPathPrefix.empty()) {
    for(int i = 0; i < morseSmaleComplexQuasi->GetNumberOfOutputPorts(); i++) {
      auto output = morseSmaleComplexQuasi->GetOutputDataObject(i);
      auto writer
