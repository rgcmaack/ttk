...
int ttk::WebSocketIO::stopServer(){
    ...
    if(this->server.is_listening()){
        ...
        {
            this->printMsg("Closing Connections",0,0,ttk::debug::LineMode::REPLACE,ttk::debug::Priority::DETAIL);
            // initiate closing
            {
                std::lock_guard<std::mutex> guard(this->mutex);
                for(con_list::iterator it = this->connections.begin(); it != this->connections.end(); ++it){
                    this->server.close(
                            *it,
            ...
            this->printMsg("Closing Connections",1,0,ttk::debug::LineMode::NEW,ttk::debug::Priority::DETAIL);
        }
        // Stop the endpoint.
        {
            this->printMsg("Terminating Server Thread",0,0,ttk::debug::LineMode::REPLACE,ttk::debug::Priority::DETAIL);
            ...
            this->printMsg("Terminating Server Thread",1,0,ttk::debug::LineMode::NEW,ttk::debug::Priority::DETAIL);
        }
    }
...
int ttk::WebSocketIO::sendString(
...
int ttk::WebSocketIO::sendBinary(
...
int ttk::WebSocketIO::sendMessage(
    const Message& msg
) const {
...
int ttk::WebSocketIO::queueMessage(
    const size_t& sizeInBytes,
    const void* data
){
...
int ttk::WebSocketIO::sendNextQueuedMessage(){
    ...
    if(nRemainingMessages>1){
        float progress = ((float)(this->nMessages-nRemainingMessages))/((float)(this->nMessages-1));
        this->printMsg(
            "Sending "+std::to_string(this->nMessages)+" queued messages",
            progress,this->msgTimer.getElapsedTime(),
            ttk::debug::LineMode::REPLACE
        );
    } else {
        this->printMsg(
            "Sending "+std::to_string(this->nMessages)+" queued messages",
            1,this->msgTimer.getElapsedTime()
        );
    }
...
int ttk::WebSocketIO::processMessageQueue(){
    ...
    this->messageQueue.emplace_front( ttk::WebSocketIO::Message("ttk_WSIO_BeginMessageSequence") );
    this->messageQueue.emplace_back( ttk::WebSocketIO::Message("ttk_WSIO_EndMessageSequence") );

    return this->sendNextQueuedMessage();
}

int ttk::WebSocketIO::processEvent(const std::string& eventName, const std::string& eventData){
    this->printMsg(
        "processEventBase:" + eventName + " -> " +eventData,
...
int ttk::WebSocketIO::on_open(websocketpp::connection_hdl hdl){
    ...
    ttk::Timer t;
    this->printMsg("Establishing Connection",0,0,ttk::debug::LineMode::REPLACE);
...
int ttk::WebSocketIO::isListening(){return 0;}
int ttk::WebSocketIO::getPortNumber(){return -1;}
int ttk::WebSocketIO::startServer(int PortNumber){return 0;}
int ttk::WebSocketIO::stopServer(){return 0;}
int ttk::WebSocketIO::sendString(const std::string& msg) const {return 0;}
int ttk::WebSocketIO::sendBinary(const size_t& sizeInBytes,const void* data) const {return 0;}
int ttk::WebSocketIO::sendMessage(const Message& msg) const {return 0;}
int ttk::WebSocketIO::queueMessage(const std::string& msg){return 0;}
int ttk::WebSocketIO::queueMessage(const size_t& sizeInBytes,const void* data){return 0;}
int ttk::WebSocketIO::queueMessage(const Message& msg){return 0;}
int ttk::WebSocketIO::clearMessageQueue(){return 0;}
int ttk::WebSocketIO::sendNextQueuedMessage(){return 0;}
int ttk::WebSocketIO::processMessageQueue(){return 0;}
int ttk::WebSocketIO::processEvent(const std::string& eventName, const std::string& eventData){return 0;}
int ttk::WebSocketIO::on_open(websocketpp::connection_hdl hdl){return 0;}
int ttk::WebSocketIO::on_close(websocketpp::connection_hdl hdl){return 0;}
int ttk::WebSocketIO::on_message(websocketpp::connection_hdl hdl, WSServer::message_ptr msg){return 0;}
