...
int ttkWebSocketIO::RequestData(
    ...
    if(neededUpdate){
        // if needed update then the client send data to the server which was parsed and is now passed on as the output of the filter
        auto output = vtkMultiBlockDataSet::GetData( outputVector );
        output->ShallowCopy( this->LastOutput );
    } else {
        // in any other case (i.e., if the input or parameters changed) then send the last input to the client
        auto input = vtkDataObject::GetData( inputVector[0] );
        this->LastInput = vtkSmartPointer<vtkDataObject>::Take( input->NewInstance() );
...
int ttkWebSocketIO::processEvent(
...
template <typename T> T jsonGetValue(
...
template <typename T> void jsonArrayToArray(
        const boost::property_tree::ptree& pt,
        const boost::property_tree::ptree::key_type& key,
        T* result
) {
    ...
    ttkUtils::stringListToDoubleVector(
        jsonGetValue<std::string>(pt,key),
        values
    );
...
bool jsonHasChild(const boost::property_tree::ptree& pt, const boost::property_tree::ptree::key_type& key) {
    return pt.find(key) != pt.not_found();
}

int ttkWebSocketIO::ParseVtkDataObjectFromJSON( const std::string& json, vtkDataObject* object ){
    ttk::Timer timer;
    this->printMsg("Parsing vtkDataObject from JSON string",0,0,ttk::debug::LineMode::REPLACE);
    ...
    for(auto it = pt.begin(); it != pt.end(); ++it) {
        ...
        if(
            !jsonHasChild(jsonVtkDataSet,"className") ||
        ...
        if( jsonGetValue<std::string>(jsonVtkDataSet,"className").compare("vtkUnstructuredGrid")!=0 ){
            this->printErr("Currently WebSocketIO only supports parsing 'vtkUnstructuredGrids'.");
            return 0;
        }

        // parse points
        {
            auto nPoints = jsonGetValue<int>(jsonVtkDataSet,"points.coordinates.nTuples");

            vtkSmartPointer<vtkPoints> points = vtkSmartPointer<vtkPoints>::New();
            points->SetNumberOfPoints(nPoints);

            jsonArrayToArray<float>(jsonVtkDataSet, "points.coordinates.data", (float *) ttkUtils::GetVoidPointer(points));
        ...
        // parse cells
        {
            auto nOffsets = jsonGetValue<int>(jsonVtkDataSet,"cells.offsetsArray.nTuples");
            ...
            jsonArrayToArray<vtkIdType>(jsonVtkDataSet, "cells.offsetsArray.data", offsetsData);

            auto nConnectivity = jsonGetValue<int>(jsonVtkDataSet,"cells.connectivityArray.nTuples");
            auto connectivity = vtkSmartPointer<vtkIdTypeArray>::New();
            connectivity->SetNumberOfTuples( nConnectivity );
            jsonArrayToArray<vtkIdType>(jsonVtkDataSet, "cells.connectivityArray.data", (vtkIdType*) ttkUtils::GetVoidPointer( connectivity ));
            ...
            for(int i=1; i<nOffsets; i++){
                cellTypesData[i-1] = vtkCellsTypeHash[ offsetsData[i]-offsetsData[i-1] ];
            }

            block->SetCells(cellTypes, cells);
        }

        auto addArraysToAssociation = [](vtkFieldData* fd, const boost::property_tree::ptree& fdJSON, const std::string& fdKey){
            // parse point data
            for(auto pda : fdJSON.get_child( fdKey )){
                ...
                auto array = vtkSmartPointer<vtkDataArray>::Take(
                    vtkDataArray::CreateDataArray(
                        jsonGetValue<int>(jsonArray,"dataType")
                    )
                );
                ...
                switch(array->GetDataType()){
                    vtkTemplateMacro(
                        jsonArrayToArray<VTK_TT>(
                            jsonArray,
                            "data",
                            (VTK_TT*) ttkUtils::GetVoidPointer( array )
                        )
                    );
                }
        ...
        addArraysToAssociation( block->GetPointData(), jsonVtkDataSet, "pointData" );
        addArraysToAssociation( block->GetCellData(), jsonVtkDataSet, "cellData" );
        addArraysToAssociation( block->GetFieldData(), jsonVtkDataSet, "fieldData" );

        this->LastOutput->SetBlock(b++, block);
    }

    this->printMsg("Parsing vtkDataObject from JSON string",1,timer.getElapsedTime());
...
int ttkWebSocketIO::SendVtkDataObject(vtkDataObject* object){
    ttk::Timer timer;
    this->printMsg("Serializing vtkDataObject",0,0,ttk::debug::LineMode::REPLACE);
    ...
    for(size_t b=0; b<nBlocks; b++){
        ...
        if(!block){
            this->printErr("Input `vtkDataObject` is not a `vtkDataSet` or a `vtkMultiBlockDataSet` containing only `vtkDataSets`.");
            return 0;
        }

        // send class name
        this->queueMessage(
            "{ \"className\": \"" + std::string(block->GetClassName()) + "\" }"
        );
        ...
        if(block->IsA("vtkPointSet")){
            ...
            this->queueMessage(
                "{"
                    "\"target\": \"points\","
                    "\"name\": \"coordinates\","
                    "\"dataType\":" +std::to_string(points->GetDataType())+ ","
                    "\"nTuples\":" +std::to_string(blockAsPS->GetNumberOfPoints()) + ","
                    "\"nComponents\": 3"
                "}"
            );
            ...
            if(cells){
                {
                    ...
                    this->queueMessage(
                        "{"
                            ...
                            "\"dataType\":" +std::to_string(VTK_ID_TYPE)+ ","
                            "\"nTuples\":" +std::to_string(connectivityArray->GetNumberOfTuples()) + ","
                            "\"nComponents\": 1"
                        "}"
                    );
                    this->queueMessage(
...

                {
                    ...
                    this->queueMessage(
                        "{"
                            ...
                            "\"name\": \"offsetsArray\","
                            "\"dataType\":" +std::to_string(VTK_ID_TYPE)+ ","
                            "\"nTuples\":" +std::to_string(offsetsArray->GetNumberOfTuples()) + ","
                            "\"nComponents\": 1"
                        "}"
                    );
                    this->queueMessage(
        ...
        // send point, cell, and field data
        {
            ...
            for(auto& attribute : attributes){
                ...
                for(size_t i=0; i<nArrays; i++){
                    auto array = attribute.second->GetArray(i);
                    if(!array)
                        continue;
                    this->queueMessage(
                        "{"
                            ...
                            "\"dataType\":"    + std::to_string(array->GetDataType()) +  ","
                            "\"nTuples\":"     + std::to_string(array->GetNumberOfTuples()) +  ","
                            "\"nComponents\":" + std::to_string(array->GetNumberOfComponents()) +
                        "}"
                    );
                    switch(array->GetDataType()){
                        vtkTemplateMacro(
                            this->queueMessage(
                                array->GetNumberOfValues()*sizeof(VTK_TT),
                                ttkUtils::GetVoidPointer(array)
                            )
                        );
                    }
                }
